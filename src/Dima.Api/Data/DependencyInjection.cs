...
{
    /// <summary>
    /// The connection string key for the Dima database connection.
    /// </summary>
    private const string DimaDb = nameof(DimaDb);
    ...
    {
        var connectionString = builder.Configuration.GetConnectionString(DimaDb)
                               ?? throw new InvalidOperationException($"Connection string '{DimaDb}' not found.");
        builder.Services
